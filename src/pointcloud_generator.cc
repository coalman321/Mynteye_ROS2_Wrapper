...
PointCloudGenerator::PointCloudGenerator(CameraIntrinsics in, Callback callback,
                                         rclcpp::Node* nodeptr, double factor,
    ...
    : in_(std::move(in)),
      ...
      factor_(factor) {
    if (frequency > 0) {
        timer = nodeptr->create_wall_timer(
            std::chrono::milliseconds((int)(1000.0 / frequency)),
            std::bind(&PointCloudGenerator::Run, this));
    }
    Start();
}

PointCloudGenerator::~PointCloudGenerator() { Stop(); }
